...
class PolyBasis:
    ...
    def __init__(self, ndim=1, nord=1, pol_type='HG'):
        ...
        # Ininializes the object
        assert pol_type in [
            'hermite', 'legendre'], 'Only Hermite and Legendre polynomials are currently supported! Please choose among ' + str(['hermite', 'legendre']) + '!'
        self._nord = nord
        self._ndim = ndim
...
class PCE(Module):
    ...
    def __init__(self, in_features: int, out_features: int, pce_order: int,
                 poly_name: str,
        ...
        self.npce = int(math.factorial(in_features+pce_order) /
                        (math.factorial(pce_order) * math.factorial(in_features)))
        self.weight = Parameter(torch.empty((out_features, self.npce), **factory_kwargs))
    ...
    def forward(self, input: Tensor) -> Tensor:
        psi_xi = Tensor(PolyBasis(self.in_features, self.pce_order, self.poly_name)(input))
